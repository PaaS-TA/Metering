...
// Return the PAASTA-API-METERING-SAMPLE root directory
var rootDir = function(dir) {
  if(dir === '/') return dir;
  try {
    if(JSON.parse(fs.readFileSync(
      path.resolve(dir, 'package.json')).toString()).name === 'paasta-api-metering-sample')
      return dir;
...
var runCLI = function() {
  ...
  commander
    ...
    // to the PAASTA-API-METERING-SAMPLE root directory
    .option(

...
/**
 * Mongo impl to bind services. Binding a service does the following: creates a
 * new user in the database (currently uses a default pwd of "password"), saves
 * the ServiceInstanceBinding info to the Mongo repository.
 * 
 * @author
 *
 */
...
public class MongoServiceInstanceBindingService implements ServiceInstanceBindingService {
	...
	public ServiceInstanceBinding createServiceInstanceBinding(CreateServiceInstanceBindingRequest request)
		...
		binding = new ServiceInstanceBinding(request.getBindingId(), request.getServiceInstanceId(), credentials, null,
				request.getAppGuid(), appOrganizationId, appSpaceId, meteringPlanId);
